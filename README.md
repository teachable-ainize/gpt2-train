...
```
$ curl --request GET 'https://train-s7p9gqcx2zy8x98lj7uv-gpt2-train-teachable-ainize.endpoint.ainize.ai/ping'
{
  "status": "Healthy"
}
```
...
Using `curl` in the terminal:
```
$ curl --request POST 'https://train-s7p9gqcx2zy8x98lj7uv-gpt2-train-teachable-ainize.endpoint.ainize.ai/predictions/gpt-2-en-small-finetune' \
  --header 'Content-Type: application/json' \
  --data-raw '{
