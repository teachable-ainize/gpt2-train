...
```
$ curl --request GET 'https://train-9s1avywlemkklfncvank-gpt2-train-teachable-ainize.endpoint.ainize.ai/ping'
{
  "status": "Healthy"
}
```
...
Using `curl` in the terminal:
```
$ curl --request POST 'https://train-9s1avywlemkklfncvank-gpt2-train-teachable-ainize.endpoint.ainize.ai/predictions/gpt-2-en-large-finetune' \
  --header 'Content-Type: application/json' \
  --data-raw '{
