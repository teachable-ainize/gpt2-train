...
```
$ curl --request GET 'https://train-12wy79gysf3p5s8e6rr6-gpt2-train-teachable-ainize.endpoint.ainize.ai/ping'
{
  "status": "Healthy"
}
```
...
Using `curl` in the terminal:
```
$ curl --request POST 'https://train-12wy79gysf3p5s8e6rr6-gpt2-train-teachable-ainize.endpoint.ainize.ai/predictions/gpt-2-en-large-finetune' \
  --header 'Content-Type: application/json' \
  --data-raw '{
