...
```
$ curl --request GET 'https://train-s3esufxpr0rvnfkg415e-gpt2-train-teachable-ainize.endpoint.ainize.ai/ping'
{
  "status": "Healthy"
}
```
...
Using `curl` in the terminal:
```
$ curl --request POST 'https://train-s3esufxpr0rvnfkg415e-gpt2-train-teachable-ainize.endpoint.ainize.ai/predictions/gpt-2-en-small-finetune' \
  --header 'Content-Type: application/json' \
  --data-raw '{
