...
```
$ curl --request GET 'https://train-di3u6pwbsuk44ro88h6m-gpt2-train-teachable-ainize.endpoint.staging.ainize.ai/ping'
{
  "status": "Healthy"
}
```
...
Using `curl` in the terminal:
```
$ curl --request POST 'https://train-di3u6pwbsuk44ro88h6m-gpt2-train-teachable-ainize.endpoint.staging.ainize.ai/predictions/gpt-2-en-small-finetune' \
  --header 'Content-Type: application/json' \
  --data-raw '{
